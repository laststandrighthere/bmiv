...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	mutex_init(&(rmi4_data->rmi4_esd_mutex));
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	if (rmi4_data->enable_wakeup_gesture) {
		dev_err(rmi4_data->pdev->dev.parent, "enable_wakeup_gesture is on\n");
		if (rmi4_data->no_sleep_setting) {
			synaptics_rmi4_reg_read(rmi4_data,
					rmi4_data->f01_ctrl_base_addr,
					&device_ctrl,
...
exit:
	...
	dev_info(rmi4_data->pdev->dev.parent, "Exit %s\n",__func__);
...
exit:
	...
	dev_info(rmi4_data->pdev->dev.parent, "TP-time TP resume finish\n");
	return 0;
}
